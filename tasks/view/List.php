...
class PageMainContent extends __PageElement {
	...
	public function setUIContents($modelFile=NULL) {
		...
		$columnsArray = array(	"tasks_id" => "Task",
								"name" => "Name",
								"description" => "Description", 
								"status" => "Status");
		
		$statusArray = array(	"table" => "tasks",
								"column" => "status");
		...
		$str = "";
		$str .= $this->setHeading("List Task", $columnsArray, 'CreateTask');
		$str .= $this->setDatatableLayout('list-table', $columnsArray);
		$str .= $this->endOfFormMessage();		
		$str .= $this->fillupList('list-table', $columnsJSON, $modelFile, 'tasks', 'BrowseTask', '', $statusJSON);
