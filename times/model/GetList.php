...
class ListDataExt extends GetList {
	...
	protected function overrideListData() {
		...
		foreach ($this->listData as $i=>$datarow) {
			...
			If ($datarow['status'] < '20')
				$this->listData[$i]['nextaction'] = "<span onclick=\"enableAction('SubmitTimeCard', 'uid={$datarow['uid']}')\"><a>$submit</a></span>";
			elseif ($datarow['status'] < '30')
				$this->listData[$i]['nextaction'] = "<span onclick=\"enableAction('ApproveTimeCard', 'uid={$datarow['uid']}')\"><a>$approve</a></span>";
			elseif ($datarow['status'] == '80')
				$this->listData[$i]['nextaction'] = "<span onclick=\"enableAction('ReleaseTimeCard', 'uid={$datarow['uid']}')\"><a>$release</a></span>";
			else
	...
	public function listRowsSelectiveFields($table, $fieldArray, $where='', $ord_by='uid', $ascdec='', $start=0, $limit=99999, $distinct_field='') {
		...
		$fieldArray = array_key_rename($fieldArray, 'totalhours', 'billablehours+nonbillablehours');
