...
class PageMainContent extends __PageElement {
	...
	public function setUIContents($modelFile=NULL) {
		...
		$columnsArray = array(	"users_id" => "User",
								"weekenddate" => "Weekend",
								"totalbillhrs" => "Billable Hours",
								"totalnonbillhrs" => "Non-Billable Hours",
								"totalhrs" => "Total Hours",
								"loweststatus" => "Status",
								"nextaction" => "Next Action");
		
		$statusArray = array(	"table" => "timesbyweekend",
								"column" => "loweststatus");
		...
		$str = "";
		$str .= $this->setHeading("List Time Cards by Week", $columnsArray, 'CreateTimeCard');
		$str .= $this->setDatatableLayout('list-table', $columnsArray);
		$str .= $this->endOfFormMessage();
		$str .= $this->fillupList('list-table', $columnsJSON, $modelFile, 'timesbyweekend', 'ListTime', '', $statusJSON);
