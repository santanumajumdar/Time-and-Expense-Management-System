...
class PageMainContent extends __PageElement {
	...
	public function setUIContents($modelFile=NULL) {
		...
		if (!empty($_GET['weekenddate']))
			$where .= " and weekenddate=\'{$_GET['weekenddate']}\'";
		
		$str = "";
		$str .= $this->setHeading("List Time Cards", $columnsArray, 'CreateTimeCard');
		$str .= $this->setDatatableLayout('list-table', $columnsArray);
		$str .= $this->endOfFormMessage();
		$str .= $this->fillupList('list-table', $columnsJSON, $modelFile, 'times', 'BrowseTimeCard', $where, $statusJSON);
