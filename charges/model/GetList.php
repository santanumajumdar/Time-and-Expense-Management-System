	...
	protected function overrideListData() {
		...
		foreach ($this->listData as $i=>$datarow) {
			...
			If ($datarow['status'] < '20')
				$this->listData[$i]['nextaction'] = "<span onclick=\"enableAction('SubmitCharge', 'uid={$datarow['uid']}')\"><a>$submit</a></span>";
			elseif ($datarow['status'] < '30')
				$this->listData[$i]['nextaction'] = "<span onclick=\"enableAction('ApproveCharge', 'uid={$datarow['uid']}')\"><a>$approve</a></span>";
			elseif ($datarow['status'] == '30')
				$this->listData[$i]['nextaction'] = "$tobeInvoiced";
			elseif ($datarow['status'] == '80')
				$this->listData[$i]['nextaction'] = "<span onclick=\"enableAction('ReleaseCharge', 'uid={$datarow['uid']}')\"><a>$release</a></span>";
			else
