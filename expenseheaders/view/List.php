...
class PageMainContent extends __PageElement {
	...
	public function setUIContents($modelFile=NULL) {
		...
		$advSearchArray = array("eh.users_id" => "User",
								"eh.projects_id" => "Project",
								"eh.weekenddate" => "Weekend",
								"eh.ubmitdate" => "Submitted On",
								"eh.approvedate" => "Verified On",
								"eh.invoicedate" => "Invoiced On",
								"totalamount" => "Total Amount",
								"eh.status" => "Status");
		...
		$str = "";
		$str .= $this->setHeading("List Expense", $advSearchArray, 'CreateExpenseHeader');
		$str .= $this->setDatatableLayout('list-table', $columnsArray);
		$str .= $this->fillupList('list-table', $columnsJSON, $modelFile, 'expenseheaders', 'BrowseExpenseHeader', '', $statusJSON);
