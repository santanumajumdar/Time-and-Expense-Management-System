...
class PageMainContent extends __PageElement {
	...
	public function setUIContents($modelFile=NULL) {
		...
		$columnsArray = array(	"accounts_id" => "Account",
								"name" => "Account Name",
								"lastbilldate" => "Last Billed Date",
								"status" => "Status");
		
		$statusArray = array(	"table" => "accounts",
								"column" => "status");
		...
		$str = "";
		$str .= $this->setHeading("List Account", $columnsArray, 'CreateAccount');
		$str .= $this->setDatatableLayout('list-table', $columnsArray);				
		$str .= $this->endOfFormMessage();

		$str .= $this->fillupList('list-table', $columnsJSON, $modelFile, 'accounts', 'BrowseAccount', '', $statusJSON);
